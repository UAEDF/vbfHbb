...
#### Start of RooFit part 
  ...
  ### QCD part
	  ...
	  ### Define cut
			cut    = TCut("mva%s>%.2f && mva%s<=%.2f"%(S.tag,S.boundaries[C],S.tag,S.boundaries[C+1]))
			cutB   = TCut("mva%s>%.2f && mva%s<=%.2f && mbbReg[%d]>70 && mbbReg[%d]<110"%(S.tag,S.boundaries[C],S.tag,S.boundaries[C+1],iS+1,iS+1))
	  ### Draw
	  		c0.cd()
			T.Draw("mbbReg[%d]>>%s"%(iS+1,h[N].GetName()),cut)
			T.Draw("mbbReg[%d]>>%s"%(iS+1,hb[N].GetName()),cutB)
	  ### Yields
  ...
  ### Model
	  ...
	  ### PDFs
	  		zPDF[N] = wBKG.pdf("signal_model_CAT%d"%Cp)
...
#			yS    = RooRealVar("yield_signal_mass%d_CAT%d"%(MASS,Cp),"yield_signal_mass%d_CAT%d"%(MASS,Cp),ySVBF.getValV()+ySGF.getValV(),0.,400.)
  ...
  ### Combined model
	  		model[N] = RooAddPdf("bkg_model_%s_CAT%d"%(opts.TF[iS],Cp),"bkg_model_%s_CAT%d"%(opts.TF[iS],Cp),RooArgList(tPDF[N],qcd_pdf[N]),RooArgList(yT[N],yQ[N]))
