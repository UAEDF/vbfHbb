...
name="BRN5p4_B80-200_CAT0-CAT6_FitPOL2-POL2"
nameU="_vbfhbb_${name}"
nameN="vbfhbb_${name}"

folder="BiasV11_limit_BRN5p4_dX0p1_B80-200_CAT0-6/output/"
home=`pwd`

declare -a masses=("125" "115" "135" "120" "130")
#declare -a masses=("115")
...
for mass in ${masses[@]}; do 
   datacard=`getDatacard ${mass} ${name}`
	...
	cmd1a="combine -M MaxLikelihoodFit -v 2 --saveNormalizations --plots --stepSize=0.05 --preFitValue=1.0 --robustFit=1 --X-rtd FITTER_NEW_CROSSING_ALGO --minimizerAlgoForMinos=Minuit2 --X-rtd FITTER_NEVER_GIVE_UP --X-rtd FITTER_BOUND --minimizerAlgo=Minuit2 --minimizerStrategy=1 --cminFallbackAlgo Minuit,0.001 --minimizerTolerance=0.00001 --minimizerToleranceForMinos=0.00001 --rMin=-1 --rMax=6 ${datacard} -n ${nameU}.mH${mass} -m ${mass} 2>&1 | tee `getLogName ${nameN}".mH"${mass} "MLFit" ${mass}`"
...
for mass in ${masses[@]}; do 
	if [ -f "combine/mlfit${nameU}.mH${mass}.root" ]; then 
		if [ ${run[7]} == true ]; then
			datacard=`getDatacard $mass ${name}`;
			cmd8a="text2workspace.py $datacard";
			cmd8b="root -l ${home}/DrawBestFit.C'(2.5,false,\"$mass\")' -q 2>&1 | tee `getLogName ${nameN} "fitplots"`";
			cmd8c="mv `getLogName ${nameN} "fitplots"` logs/";
			cmd8=($cmd8a $cmd8b $cmd8c)
		fi
...
#		cmd12b="${home}/PrintChannelCompatibility.py ${mass} 2>&1 | tee `getLogName ${nameN} "PrintChannelCompatibility" ${mass}`";
		cmd12c="mv channelComp_mH${mass}{,_2}.{pdf,png} plots/";
		cmd12d="mv `getLogName ${nameN} "PlotChannelCompatibility" ${mass}` logs/";
#		cmd12e="mv `getLogName ${nameN} "PrintChannelCompatibility" ${mass}` logs/";
#		cmd12=($cmd12a $cmd12b $cmd12c $cmd12d $cmd12e)
		cmd12=($cmd12a $cmd12c $cmd12d)
	fi
