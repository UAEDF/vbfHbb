...
def INTERNALstyleHist(h,i,markers,colours,TYPE):
	...
	h.GetYaxis().SetRangeUser(123.01,134.02)
	return h
...
def mkTemplateFunctions():
	...
	can = TCanvas('profiles','profiles',1800,750)
        can.Clear()
	can.Divide(2,1)
	...
	for iTag,Tag in enumerate(opts.catTags):
		...
		l1('Running for %s'%Tag)
                ...
                leg.SetTextFont(42)
                leg.SetY1(leg.GetY2()-1*leg.GetTextSize()*1.25)
		
                l2("Running for <mbbReg>")
                fout.cd()
                hDat[Tag] = TProfile("hProf_sel%s_%s"%(Tag,"Data"),"hProf_sel%s_%s"%(Tag,"Data"),50,-1,1)
                h = hDat[Tag]
		h = INTERNALstyleHist(h,0,markers,colours,"Data")
                #hQcd[Tag] = TProfile("hProf_sel%s_%s"%(Tag,TYPE),"hProf_sel%s_%s"%(Tag,TYPE),8,-1,1)
                #h = hQcd[Tag]
		#h = INTERNALstyleHist(h,0,markers,colours,TYPE)
	#	can.cd()
		for fin in fins:
                    #if "QCD" in fin.GetName(): h = hQcd[Tag]
                    #else: h = hDat[Tag]
                    #fout.cd()
                    can.cd(1 if 'NOM' in Tag else 2)
		    l4('Sample: %s'%fin.GetName())
                    CUT1 = "mbbReg[%d] > %.1f && mbbReg[%d] < %.1f"%(1 if 'NOM' in Tag else 2,80,1 if 'NOM' in Tag else 2,200)
		    ...
		    tin = fin.Get("Hbb/events")
                    ...
                    tin.Draw("mbbReg[%d]:%s>>%s"%(1 if 'NOM' in Tag else 2,"mvaNOM" if 'NOM' in Tag else "mvaVBF",h.GetName()),TCut(CUT1)*TCut(CUT2),"prof")
                    print h.GetEntries()
                    #tin.Draw("mbbReg[%d]:%s>>+%s"%(1 if 'NOM' in Tag else 2,"mvaNOM" if 'NOM' in Tag else "mvaVBF",h.GetName()),TCut(CUT1)*TCut(CUT2),"prof")
	            ...
	            h.Write(h.GetTitle(),TH1.kOverwrite)
                    ...
                    gPad.SetGrid(0,1)
                    gPad.RedrawAxis()
                    #hQcd[Tag].SetMarkerSize(1.2)
                    #hQcd[Tag].SetMarkerStyle(20)
                    #hQcd[Tag].SetMarkerColor(kBlack)
                    hDat[Tag].GetXaxis().SetTitleOffset(1.0)
                    hDat[Tag].DrawCopy()
            #        hQcd[Tag].DrawCopy("same")
                    epave("Set %s selection"%("A" if 'NOM' in Tag else "B"),gPad.GetLeftMargin()+0.01,1.-0.5*gPad.GetTopMargin(),"left",1,0.048)
        ...
        obj = can.FindObject("TFrame")
        obj.Delete()
        for i in can.GetListOfPrimitives():
            print i.GetName()

        can.SaveAs("mbbRegProfile.pdf")
# clean
