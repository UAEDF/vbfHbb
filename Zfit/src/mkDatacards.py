...
# Print card
 ...
 ## files
		...
		fout.write("%-15s %-15s *   %s/%-30s %-30s\n"%("shapes","zjets   ","root",sfname,"w:signal_model_$CHANNEL"))
...
#			fout.write("%-8s"%("%g"%yGF[i][j]))
		...
		fout.write("\n"+"-"*100+"\n")
  ## Z (anti)cor 
                fout.write("%-25s %-8s %-10.1f %-10s %-10s %-10.1f %-10s %-10s %-10.1f %-10s %-10s\n"%("Z_accep_anticor","lnN",1.1,"-","-",1.0,"-","-",0.9,"-","-"))
  ...
  ## QCD part
		for i in CATS: 
			...
			for j in CATS: 
				if i==j: fout.write("%-10s %-10s %-10s"%("-","%g"%uncertainties['qcd_norm'],"-"))
				else:    fout.write("%-10s %-10s %-10s"%("-","-","-"))
			fout.write("\n")
		fout.write("-"*100+"\n")
  ## T part
		for i in CATS: 
			...
			for j in CATS: 
				if i==j: fout.write("%-10s %-10s %-10s"%("-","-","%g"%1.2))
				else:    fout.write("%-10s %-10s %-10s"%("-","-","-"))
			fout.write("\n")
		fout.write("-"*100+"\n")
  ## scale/res part
