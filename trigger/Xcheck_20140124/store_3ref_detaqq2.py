...
def main():
	...
	for i in range(4):
		...
		for j in tags:
			...
			if h==None:
				...
				h.Sumw2()
				events.Draw("dEtaqq[2]>>h%i%s"%(i,j),"(%s %s %s)*(%.10f)"%(cutstring,"&& (triggerResult[9]==49)" if 'num' in j else "","&& (triggerResult[14]==49)" if "40" in j else ("&& (triggerResult[15]==49)" if "80" in j else ""),scale[i]))
	...
	for jj,j in enumerate(rtags): 
		...
		grat[j].GetXaxis().SetTitle('#Delta#eta_{q#bar{q}} (b-lik VBF)')
	...
	c.SaveAs("trigger_1d_detaqq2.png")
	c.Close()
