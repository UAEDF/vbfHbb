...
def main():
	...
	print "LOADING FILES..."
	f = [None]*4
	f[0] = TFile("/data/UAData/autumn2013.old/flatTree_QCD100_reformatted.root")
	f[1] = TFile("/data/UAData/autumn2013.old/flatTree_QCD250_reformatted.root")
	f[2] = TFile("/data/UAData/autumn2013.old/flatTree_QCD500_reformatted.root")
	f[3] = TFile("/data/UAData/autumn2013.old/flatTree_QCD1000_reformatted.root")
	f5 = TFile("store_3ref_ht.root","update")

	scale = [18280./x for x in [4.827581,98.051,3631.531688,67862.073529]]
	tags = ["num40","den40","num80","den80","numnoref","dennoref"]
	cutstring = "(runNo==1) && (jetPt[1]>35.) && (dEtaqq[2]>3.5) && (mqq[2]>600.)"
	...
	for i in range(4):
		...
		for j in tags:
			...
			if h==None:
				h = TH1F("h%i%s"%(i,j),"h%i%s"%(i,j),50,0,1000)
				h.Sumw2()
				events.Draw("ht>>h%i%s"%(i,j),"(%s %s %s)*(%.10f)"%(cutstring,"&& (triggerResult[9]==49)" if 'num' in j else "","&& (triggerResult[14]==49)" if "40" in j else ("&& (triggerResult[15]==49)" if "80" in j else ""),scale[i]))
	...
	for jj,j in enumerate(rtags): 
		...
		grat[j].GetXaxis().SetTitle("ht [GeV/c]")
