...
hsig_btagRank = TH1F("hsig_btagRank",";CSV b-tag value;N / (1.0)",4,-0.5,3.5)
hsig_eta = TH1F("hsig_eta",";#eta;N / (0.1)",100,-5,5)
hsig_etaRank = TH1F("hsig_etaRank",";#eta rank;N / (1.0)",4,-0.5,3.5)

hbkg_btag = TH1F("hbkg_btag",";CSV b-tag value;N / (0.1)",110,-10,1)
hbkg_btagRank = TH1F("hbkg_btagRank",";CSV b-tag value;N / (1.0)",4,-0.5,3.5)
hbkg_eta = TH1F("hbkg_eta",";#eta;N / (0.1)",100,-5,5)
hbkg_etaRank = TH1F("hbkg_etaRank",";#eta rank;N / (1.0)",4,-0.5,3.5)

#v = ["jetPt","jetEta","jetBtag","dEtaqq","mqq","mbbReg","b1","b2","triggerResult","btagIdx","etaIdx"]
#t.SetBranchStatus("*",0)
#for iv in v: 
#    t.SetBranchStatus(iv,1)
    ...
    if event.jetBtag[event.b1[0]]>0.244 and event.jetBtag[event.b2[0]]>0.244 and event.jetPt[3]>40.0 and event.jetPt[2]>50.0 and event.jetPt[1]>70.0 and event.jetPt[0]>80.0 and event.dEtaqq[1]>2.5 and event.mqq[1]>250:
        if event.triggerResult[0]==TRUE0 or event.triggerResult[1]==TRUE0:
            for jid in range(4):
                ...
                btagRank = event.btagIdx[jid]
                etaRank = event.etaIdx[jid]
                if abs(id)==5:
                    hsig_btag.Fill(btag)
...
fsave = TFile("blikelihoodAlt.root","recreate")
for h in [hsig_btag,hsig_btagRank,hsig_eta,hsig_etaRank]:
    h.Write()
for h in [hbkg_btag,hbkg_btagRank,hbkg_eta,hbkg_etaRank]:
    h.Write()
...
##s_btag = THStack()
##s_btag.Add(hbkg_btag)
##s_btag.Add(hsig_btag)
##s_btagRank = THStack()
##s_btagRank.Add(hbkg_btagRank)
##s_btagRank.Add(hsig_btagRank)
##s_eta = THStack()
##s_eta.Add(hbkg_eta)
##s_eta.Add(hsig_eta)
##s_etaRank = THStack()
##s_etaRank.Add(hbkg_etaRank)
##s_etaRank.Add(hsig_etaRank)
