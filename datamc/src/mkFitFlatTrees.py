...
def mkFitFlatTrees():
	...
	for s in sorted(allsamples.itervalues(),key=lambda x: (x['tag'] if not 'QCD' in x['tag'] else x['tag'][0:3],float(x['tag'][3:].replace('_sNOM','').replace('_sVBF','')) if 'QCD' in x['tag'] else 1)):
	   ...
	   # veto regex in opts.nosample
		...
		# extra vetoes
		if 'VBF1' in s['fname'] and (not any([('VBF' in trg or 'TPV' in trg) for trg in opts.trigger])): continue
		if ('BJetPlus' in s['fname'] or 'MultiJet' in s['fname']) and any([('VBF' in trg or 'TPV' in trg) for trg in opts.trigger]): continue
