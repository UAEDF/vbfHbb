...
void CreateDataTemplates(double dX, float XMIN, float XMAX, int CATMIN, int CATMAX, int BRN_ORDER_NOM, int BRN_ORDER_VBF, TString OUTPATH, TString TR, TString BR, bool FitWithSignal, int MASS, float XMAXDIFF=0)
{
  ...
  const int TRFdsize(7);
  TString TRFKEYS[TRFdsize] = {"POL1","POL2","POL3","FixedPOL1","FixedPOL2","FixedPOL3","EXPO"};
  TString TRFVALS[TRFdsize] = {"@1*@0+1","@2*@0*@0+@1*@0+1","@3*@0*@0*@0+@2*@0*@0+@1*@0+1","@2*@0+@1","@3*@0*@0+@2*@0+@1","@4*@0*@0*@0+@3*@0*@0+@2*@0+@1","exp(@2*@0+@1)"};
  ...
  if (withTRF) {
		if (XMAXDIFF==0.) TFile *fTransfer = TFile::Open(TString::Format("%s/output/transferFunctions_B%.f-%.f.root",OUTPATH.Data(),XMIN,XMAX).Data());
		else              TFile *fTransfer = TFile::Open(TString::Format("%s/output/transferFunctions_B%.f-%.f%.f.root",OUTPATH.Data(),XMIN,XMAX,XMAX+XMAXDIFF).Data());
		TF1 *transFunc;
  }
  ...
  for(int isel=0;isel<NSEL;isel++) {
    ...
    if (withBRN) { tBRN=TString::Format("_Alt%s",BR.Data()); }
	 ...
	 else if (BR(0,3)=="brn") NPAR = atoi(BR(3,1).Data());
	 else NPAR = 2; 
    ...
    RooArgSet brn_params[NCATSEL];
	 ...
	 for(int icat=0; icat<NCAT[isel]; icat++) {
		 ...
		 printf("\nVariable: x(%s): %d - %d\n",("mbbReg"+CATSTRING).Data(),XMIN,XMAX);
		 float p[5], e[5];
		 ...
		 if (icat!=0 && withTRF) {
      	...
      	// --- Must be consistent with TransferFunctions.C
			for (int ipar=0; ipar<NPARS; ipar++) {
				...
				trans_p[ipar] = new RooRealVar(TString::Format("trans_%s_p%i_CAT%d",sTRF.Data(),ipar,counter),TString::Format("trans_%s_p%i_CAT%d",sTRF.Data(),ipar,counter),p[ipar],-0.1.,0.1);
         ...
         if (TR=="EXPO")      { transfer[icat] = new RooGenericPdf(TString::Format("transfer_%s_CAT%d",TR.Data(),counter).Data(),TRFVALS[TRFmatch].Data(),RooArgList(x,*trans_p[0],*trans_p[1])); }
      ...
      roohist_blind[icat] = new RooDataHist(name,name,x,hBlind);
		...
		TString MSTRING="";
		MSTRING = TString::Format("_m%d",MASS);
		TString MASSSTRING="";
		MASSSTRING = TString::Format("_mass%d",MASS);
...
//		int yield = 0;	
//		for( int i=0; i<hresid[icat]->GetN(); i++) {
...
//		if ( TR=="" || icat==0 ) {
//		cout << "\033[0;38;5;22m";
//	 	cout << "[ERROR CALC] " << hresid->GetN() << " should be equal to " << roohist[icat]->numEntries() << endl;
//		cout << "[ERROR CALC] " << "NBINS: " << NBINS << endl;
//		cout << "[ERROR CALC] " << "NPARS: " << NPAR+1 << endl;
//		cout << "[ERROR CALC] " << "CHI2:  " << Sum2 << endl;
//		cout << "[ERROR CALC] " << "CHI2P: " << Sum2Poisson << endl;
//		cout << "[ERROR CALC] " << "CHI2 / NDOF: " << Sum2 / (NBINS - (NPAR+1)) << endl;
//		cout << "[ERROR CALC] " << "CHI2P / NDOF: " << Sum2Poisson / (NBINS - (NPAR+1)) << endl;
//		cout << "[ERROR CALC] " << "Probability chi squared: " << TString::Format("%.4f",TMath::Prob(Sum2,        (NBINS-(NPAR+1))))                 << endl;
//		cout << "[ERROR CALC] " << "                         " << TString::Format("%.4f",TMath::Prob(Sum2Poisson, (NBINS-(NPAR+1)))) << " (Poisson)" << endl;
//		cout << "[ERROR CALC] " << "Minimized -log(L): " << res->minNll() << "\033[m" << endl << endl;
//		}
      ...
      for(int ib=0;ib<=NPAR;ib++) {
		...
		if (withTRF) {
			for (int ipar=0; ipar<NPARS; ipar++) {
				if (icat>0) trans_p[ipar]->setConstant(((TR(0,5)!="Fixed")&&(TR!="EXPO")) ? kFALSE : kTRUE);
