...
## Baseline
		...
		line.SetLineWidth(1)
		line.SetLineStyle(2)
		line.SetMinimum(0.90)
		line.SetMaximum(1.1)
		line.GetXaxis().SetTitle("m_{b#bar{b}} (GeV)")
...
#		L0.SetHeader("%s selection"%S.label.replace('NOM','Set A').replace('PRK','Set B'))
		...
		L0.SetBorderSize(0)
		L0.SetTextFont(42)
		L0.SetTextSize(gStyle.GetPadTopMargin()*0.70)
...
#### Other CATs
  ...
  ## Mbb plot
				...
				CN0.cd()
                                if iS==1:
		                    line.SetMinimum(0.85)
                    	    	    line.SetMaximum(1.15)
                                    ndivy=508
				h.GetYaxis().SetNdivisions(ndivy)
				g.GetYaxis().SetNdivisions(ndivy)
				ga.GetYaxis().SetNdivisions(ndivy)
				line.GetYaxis().SetNdivisions(ndivy)
				h.Draw("same,PE")
  ...
  ## Ratio plot
				...
				g.Draw("sameE3")
				r.Draw("same")
				L1 = TLegend(gStyle.GetPadLeftMargin()+gStyle.GetPadTopMargin()*0.3333+0.01,gStyle.GetPadBottomMargin()+gStyle.GetPadTopMargin()*0.3333+0.007,0.8,0.4)
				L1.SetHeader("CAT%d / CAT%d"%(Cp,sum(SC.ncats[0:iS])))#S.label.replace('NOM','Set A').replace('PRK','Set B'),
                                L1.SetNColumns(2)
				L1.SetFillStyle(0)
				L1.SetBorderSize(0)
				L1.SetTextFont(42)
