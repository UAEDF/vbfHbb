...
for fname in args:
	...
	w = fopen.Get("w")
	keys = [x for x in os.path.split(fname)[1].split("_") if 'root' in x and not 'BRN' in x]
	if len(keys)>0: key = keys[0].replace('.root','')
	else: key=""
	print "%42s |"%os.path.split(fname)[1],
	print "%5s |"%key,
	for i in range(6):
