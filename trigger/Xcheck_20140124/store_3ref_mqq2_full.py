...
def main():
	...
	print "LOADING FILES..."
	f = [None]*4
	f[0] = TFile("/data/UAData/autumn2013.old/flatTree_QCD100.root")
	f[1] = TFile("/data/UAData/autumn2013.old/flatTree_QCD250.root")
	f[2] = TFile("/data/UAData/autumn2013.old/flatTree_QCD500.root")
	f[3] = TFile("/data/UAData/autumn2013.old/flatTree_QCD1000.root")
	f5 = TFile("store_3ref_mqq2_full.root","update")

	scale = [18280./x for x in [4.827581,98.051,3631.531688,67862.073529]]
	tags = ["num40","den40","num80","den80","numnoref","dennoref"]
	cutstring = "(runNo==1) && (jetPt[1]>35.) && (dEtaqq[2]>3.5) && (ht>100.)"
	...
	for i in range(4):
		...
		for j in tags:
			...
			if h==None:
				h = TH1F("h%i%s"%(i,j),"h%i%s"%(i,j),100,0,2000)
				h.Sumw2()
				events.Draw("mqq[2]>>h%i%s"%(i,j),"(%s %s %s)*(%.10f)"%(cutstring,"&& (triggerResult[9]==1)" if 'num' in j else "","&& (triggerResult[14]==1)" if "40" in j else ("&& (triggerResult[15]==1)" if "80" in j else ""),scale[i]))
	...
	for jj,j in enumerate(rtags): 
		...
		grat[j].GetXaxis().SetTitle('m_{q#bar{q}} (b-lik VBF) [GeV/c^2]')
	...
	c.SaveAs("trigger_1d_mqq2_full.png")
	c.Close()
