void TransferFunctions(float BND1,float BND2,float BND3,float XMIN,float XMAX)
{
  ...
  for(int isel=0;isel<NSEL;isel++) {
    ...
    can1[isel]->cd();
    leg->Draw();
	 can1[isel]->SaveAs(TString::Format("plots/transfer/%s.pdf",can1[isel]->GetName()));
	 can2[isel]->SaveAs(TString::Format("plots/transfer/%s.pdf",can2[isel]->GetName()));
  }
  can->SaveAs(TString::Format("plots/transfer/%s.pdf",can->GetName()));
  outf->Close();
  delete can;
}
