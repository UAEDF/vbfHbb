...
#
	...
	mg2.add_option('--MASS',help=colours[5]+'Signal masspoint: 125'+colours[0],default=[125],type='str',action='callback',callback=optsplitfloat)
...
# Load yields
	...
	for i in range(sum(SC.ncats)):
		...
		for j,jmass in enumerate(range(100,155,5)):
			if j==0: 
				yVBF += [[]]
				yGF  += [[]]
 ...
 ## top info
  ...
  ## fixed parts
		...
		for il,l in enumerate(labels):
			...
			for i in CATS: 
                                if jmass>=115 and jmass<=135:
                		    k = j-3 if len(uncertainties[l][0])==5 else i if len(uncertainties[l][0])==ncats else 0
                                elif jmass<115:
                                    k = 0 if len(uncertainties[l][0])==5 else i if len(uncertainties[l][0])==ncats else 0
                                elif jmass>135:
                                    k = 4 if len(uncertainties[l][0])==5 else i if len(uncertainties[l][0])==ncats else 0
                                else:
                                    sys.exit("Unhandled masspoint: %.f"%jmass)
